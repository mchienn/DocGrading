...
| BR-06 | Chỉ nhận file có magic bytes và MIME là PDF, tối đa 50 MB và 100 trang, không mã hóa/mật khẩu, không attachment hoặc JavaScript nhúng. Traversal active-content có budget độc lập với page-tree traversal; hết budget phải fail closed bằng `PDF_SCAN_LIMIT`, không được báo sai `PDF_ACTIVE_CONTENT`. |
| BR-07 | Một trang bị coi là nghi scan khi ảnh raster phủ từ 80% diện tích trang và có dưới 30 ký tự text hữu ích. File có bất kỳ trang nghi scan nào bị từ chối; trang trắng thật không tính là trang scan. Budget 10.000 geometry operations áp dụng độc lập cho từng trang, vẫn tính mọi Form lồng trong trang. Clipping Bézier `c`/`v`/`y` dùng bounding box của các control point làm conservative upper bound nên không thể che giấu scan; compound path, `W*`, path chỉ gồm đoạn thẳng nhưng không lồi/tự cắt/suy biến hoặc clip vượt giới hạn vẫn fail closed thành `PDF_MALFORMED`, không bao giờ được coi là 0% coverage. |
...
| AT-05 | File sai định dạng, quá 50 MB, quá 100 trang, có mật khẩu, active content, clipping đã áp dụng nhưng không thể đo chính xác và cũng không thể conservative-bound an toàn, hoặc trang nghi scan bị từ chối với lý do và trang liên quan; không enqueue evaluator. |
...
| File/evidence | `GET /document-versions/{version_id}/download`, `GET /submissions/{submission_id}/evidence` | Download kiểm tra quyền trước khi trả redirect/URL ký tối đa 5 phút. Evidence endpoint chỉ dành cho Admin hoặc Teacher phụ trách, trả finding của DocumentVersion mới nhất cùng `document_ir_id`, `element_id`, page và bbox được resolve từ Document IR đã lưu; không trả text, raw IR, PDF bytes hoặc storage key. |
