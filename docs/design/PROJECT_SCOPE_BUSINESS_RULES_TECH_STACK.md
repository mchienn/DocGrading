...
- `ReviewDraft`, `ReviewDecision`, `ReviewLock`, `PublishedResultVersion`, `ReviewRequest`.
...
| BR-06 | Chỉ nhận file có magic bytes và MIME là PDF, tối đa 100 MB raw và 100 trang, không mã hóa/mật khẩu. Budget decoded stream độc lập là 50 MB; tăng giới hạn raw không làm tăng ngân sách giải nén. JavaScript, Launch, EmbeddedFiles, XFA, form có field thật, `/GoToE` và remote action tự động qua `/OpenAction`, `/AA` hoặc chuỗi `/Next` vẫn bị từ chối. Link annotation do người dùng click với `/URI` được chấp nhận; `/GoToR` chỉ được chấp nhận khi là action `/A` trực tiếp của `/Link` nằm trực tiếp trong mảng `/Annots` của page, có `/F` là PDF string hoặc `/Filespec` thật không chứa active content. Backend không resolve hoặc fetch target. `/StructTreeRoot` dùng traversal riêng có giới hạn và chống chu kỳ để metadata accessibility không chiếm budget active-content chung; `/AF`, `/EF` và active descendant bên trong vẫn bị từ chối. Mỗi lần validation lưu report có mã lỗi an toàn, disposition, page, bbox khi có và repair action; lỗi parser/hạ tầng ngoài taxonomy PDF phải là `PROCESSING_FAILED`, không được gán `PDF_MALFORMED`. Strict parser failure được chạy lại một lần bằng lenient parser nhưng vẫn giữ nguyên mọi budget và security check; chỉ kết quả vượt toàn bộ pipeline mới được nhận với warning `PDF_STRUCTURE_RECOVERED`. |
| BR-07 | Validator preflight page content, toàn bộ Form XObject stream và font resource stream (`/FontFile*`, `/ToUnicode`, stream-backed `/Encoding`) trước khi trích text; mỗi stream duy nhất chỉ bị tính một lần vào decoded budget trên toàn tài liệu. Trang có ít nhất 30 ký tự hữu ích bỏ qua raster geometry nhưng vẫn áp dụng raw, decoded, page, structure và active-content budgets. Chỉ trang ít text chạy scan geometry: ảnh raster phủ từ 80% diện tích trang làm trang nghi scan và bị từ chối; trang trắng thật không tính là scan. Budget 10.000 geometry operations áp dụng độc lập cho từng trang được phân tích, kể cả Form lồng. Clipping Bézier `c`/`v`/`y` với `W` dùng bounding box control point làm conservative upper bound; path lồi đơn giản với `W` hoặc `W*` được đo chính xác. Geometry không thể phân tích an toàn bị từ chối bằng `PDF_SCAN_ANALYSIS_UNSUPPORTED`, không bị báo sai là malformed. |
| BR-08 | File gốc được lưu ngoài web root. Presigned POST chỉ ghi vào staging key. Completion phải kiểm tra metadata, rồi `CopyObject` có `CopySourceIfMatch` theo ETag sang sealed key ngẫu nhiên không xuất hiện trong credential upload; chỉ sealed key được lưu cho worker và download để presign còn hạn không thể ghi đè file đã nhận. Truy cập file dùng quyền theo object và URL ký có hạn 5 phút. Không lưu PDF blob trong PostgreSQL. |
...
| BR-11 | Mọi finding tự động phải có criterion, phương pháp, mô tả, page number, text quote và tọa độ hoặc section anchor. File integrity đối chiếu label URL hiển thị với target `/URI` hoặc `/GoToR` mà không resolve/fetch mạng; mismatch tạo finding và evidence anchor theo bbox của link, target không đủ dữ liệu chuyển review thủ công. Citation evaluator deterministic nhận diện bibliography, numeric/author-year marker, DOI/arXiv ID và liên kết mention-reference; Crossref chỉ là provider identity, timeout/retry/cache bounded, lỗi hoặc arXiv chưa có adapter là `UNRESOLVED`, không tự coi là citation giả. Không xác minh được anchor thì không hiển thị finding như bằng chứng; criterion chuyển `NEEDS_MANUAL_REVIEW`. |
...
| BR-22 | Nộp lại tạo phiên bản mới và phân tích lại bằng snapshot áp dụng tại thời điểm nộp. Có thể so sánh hai phiên bản bất kỳ được chọn trong cùng Submission theo score, review decision, comment và số evidence đã lưu; không chấm lại hoặc diff pixel toàn bộ PDF. |
...
| AT-05 | File sai định dạng, quá 100 MB raw, vượt 50 MB decoded, quá 100 trang, có mật khẩu, active content, scan geometry không thể xác minh an toàn hoặc trang nghi scan bị từ chối với diagnostic code, disposition, repair action và page/bbox khi xác định được; không enqueue evaluator. |
| AT-06 | Sau upload, sinh viên thấy đúng các trạng thái Validating, Queued, Processing, Awaiting review, Published hoặc lỗi; không có spinner vô hạn và phần trăm giả. Khi validator có diagnostic, sinh viên thấy nguyên nhân an toàn, cách sửa và mở đúng page/region trên PDF viewer dùng chung. |
| AT-07 | Upload lặp hoặc redelivery từ queue không tạo kết quả/finding trùng. Commit `QUEUED`, API restart hoặc broker lỗi không làm mất wake-up nhờ dispatch outbox; dừng worker giữa job rồi khởi động lại không làm mất job; retry dừng sau ba lần. Worker PDF mặc định concurrency 2, prefetch 1 và recycle child định kỳ để giới hạn số file 100 MB đồng thời và thu hồi bộ nhớ parser. |
| AT-08 | Document IR schema 2 lưu text theo trang, section, requirement, use case, table, link annotation và coordinate. Mỗi evidence mở đúng trang; anchor sai bị đánh dấu thủ công thay vì hiển thị như hợp lệ. Trang có text-table discovery vượt work budget phải bỏ qua heuristic tốn kém, giữ text/coordinate và đánh dấu phần tử cần review thay vì loại toàn bộ PDF. |
...
| AT-16 | Nộp lại giữ phiên bản cũ, tạo phiên bản mới và cho phép so sánh hai phiên bản bất kỳ trong cùng Submission từ dữ liệu đã lưu mà không trộn hoặc tính lại kết quả. |
...
- Authentication dùng opaque session cookie. Mọi mutation yêu cầu CSRF token. Backend kiểm tra role và quyền theo từng Course, Assignment, Submission hoặc Result; tài nguyên không tồn tại trả `404`, người dùng đã xác thực nhưng ngoài phạm vi object trả `403` theo dependency authorization dùng chung. Ngoại lệ đã chốt: Teacher truy cập object thuộc Course khác nhận `404` để không lộ sự tồn tại của object.
- Login chuẩn hóa email, serialize mỗi email bằng PostgreSQL advisory lock rồi khóa row `User`. Năm lần sai trong cửa sổ cố định 15 phút khóa login 15 phút; login đúng sau khi hết khóa xóa bộ đếm. Email không tồn tại, tài khoản bị Admin khóa và tài khoản đang bị khóa tạm đều chạy Argon2id dummy rồi trả cùng lỗi `401`, tránh lộ account qua nội dung hoặc timing dễ phân biệt.
...
- Resource có chỉnh sửa đồng thời trả `revision`. Mặc định mutation gửi `If-Match: "rev-{revision}"` và revision cũ trả `412 Precondition Failed`. Riêng autosave `PUT /submissions/{submission_id}/review-draft` gửi `revision` cùng `document_version_id` trong JSON để server nhận diện exact retry của full replacement và chặn write qua lần nộp mới; precondition cũ hoặc khác version trả `409 Conflict`.
...
| Submission | `GET /assignments/{assignment_id}/submissions`, `POST /assignments/{assignment_id}/uploads/presign`, `POST /document-versions/{version_id}/complete`, `GET /submissions/{submission_id}`, `GET /submissions/{submission_id}/versions`, `GET /submissions/{submission_id}/versions/compare` | Upload hai bước: presign yêu cầu `Idempotency-Key`, SHA-256 và client hints, trả presigned POST tối đa 5 phút khóa đúng staging key/MIME/size; completion kiểm tra metadata, conditional-copy đúng ETag sang sealed key không được presign rồi tạo hoặc trả lại đúng một AnalysisJob. Upload trùng theo BR-09 trả resource hiện có mà không cấp credential ghi đè. Danh sách version trả trạng thái xử lý/publication; compare chỉ đọc snapshot, draft và decision đã lưu, đồng thời kiểm tra owner Student hoặc quyền Course của Teacher/Admin. |
| File/evidence | `GET /document-versions/{version_id}/download`, `GET /document-versions/{version_id}/validation-report`, `GET /submissions/{submission_id}/evidence` | Download kiểm tra quyền trước khi trả redirect/URL ký tối đa 5 phút. Validation report dùng cùng quyền đọc DocumentVersion: Student sở hữu Submission, Teacher phụ trách Course hoặc Admin; response chỉ trả outcome và tối đa 25 diagnostic có code, disposition, page/bbox, metrics giới hạn và repair action, không trả raw parser content. Evidence endpoint chỉ dành cho Admin hoặc Teacher phụ trách, trả finding của DocumentVersion mới nhất cùng `document_ir_id`, `element_id`, page và bbox được resolve từ Document IR đã lưu; khi rubric bật citation criterion, response thêm counts identity/linkage và bounded issue metadata; không trả raw provider payload, text ngoài snippet cần review, PDF bytes hoặc storage key. |
| Job | `GET /analysis-jobs/{job_id}`, `POST /analysis-jobs/{job_id}/retry`, `GET /operations/analysis-jobs`, `GET /operations/analysis-jobs/{job_id}`, `POST /operations/analysis-jobs/{job_id}/retry` | Trạng thái job là `QUEUED`, `RUNNING`, `DONE`, `ERROR`; retry dùng lại row và tuân BR-10. Các endpoint `/operations/*` chỉ dành cho Admin, hỗ trợ xem/retry mọi job; Teacher chỉ xem/retry job thuộc Course mình phụ trách qua endpoint cũ; Student chỉ xem job của mình và không retry. Frontend poll, không giả lập phần trăm. |
...
| Operations | `GET/POST /users`, `GET/PATCH /users/{user_id}`, `GET /operations/audit-events`, `GET /operations/dashboard`, `GET /notifications`, `PATCH /notifications/{notification_id}/read`, `PATCH /notifications/read`, `GET /metrics` | Admin tạo account `ACTIVE` bằng email chuẩn hóa, display name, mật khẩu 12–512 ký tự băm Argon2id và một hoặc nhiều role; create trả `201` cùng `Location`, không trả password/hash và chỉ audit role/status. User patch gửi `If-Match: "rev-{revision}"`; revision cũ trả `412`. Admin xem audit có filter/pagination và dashboard aggregate không cache. User/job/audit/dashboard dùng explicit response allowlist; audit chỉ trả field đã biết an toàn theo resource, field lạ bị redact. Mỗi actor chỉ list/mark-read Notification của chính mình. Danh sách Notification có pagination và filter `unread`; single/bulk mark-read idempotent. Notification chỉ chứa `id`, type, reference-ID payload, `read_at`, `created_at`; không trả secret hoặc nội dung tài liệu. `/metrics` nằm ngoài prefix `/api/v1`, yêu cầu Admin session và trả Prometheus text gồm số job theo status, queue depth cùng tuổi trung bình của job đang active; không trả identifier hoặc dữ liệu tài liệu. |
...
2. Browser upload trực tiếp lên staging key rồi gọi completion. Completion chỉ trả `202` sau khi conditional-copy đúng ETag sang sealed key dành cho worker/download; response gồm `submission_id`, `document_version_id`, `analysis_job_id` và trạng thái job.
...
| Citation identity | Crossref REST API qua stdlib adapter | Chỉ gọi fixed Crossref endpoint cho DOI hoặc bounded bibliographic search; DOI/arXiv parsing deterministic, timeout/retry/cache bounded, không fetch arbitrary PDF URL và không dùng LLM. |
