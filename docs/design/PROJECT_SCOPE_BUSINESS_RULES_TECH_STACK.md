...
| Operations | `GET /users`, `GET/PATCH /users/{user_id}`, `GET /operations/audit-events`, `GET /operations/dashboard`, `GET /notifications`, `PATCH /notifications/{notification_id}/read`, `PATCH /notifications/read` | Admin quản lý account, xem audit có filter/pagination và dashboard aggregate không cache. User/job/audit/dashboard dùng explicit response allowlist; audit chỉ trả field đã biết an toàn theo resource, field lạ bị redact. Mỗi actor chỉ list/mark-read Notification của chính mình. Danh sách Notification có pagination và filter `unread`; single/bulk mark-read idempotent. Notification chỉ chứa `id`, type, reference-ID payload, `read_at`, `created_at`; không trả secret hoặc nội dung tài liệu. |
