...
| BR-11 | Mọi finding tự động phải có criterion, phương pháp, mô tả, page number, text quote và tọa độ hoặc section anchor. File integrity đối chiếu label URL hiển thị với target `/URI` hoặc `/GoToR` mà không resolve/fetch mạng; mismatch tạo finding và evidence anchor theo bbox của link, target không đủ dữ liệu chuyển review thủ công. Không xác minh được anchor thì không hiển thị finding như bằng chứng; criterion chuyển `NEEDS_MANUAL_REVIEW`. |
