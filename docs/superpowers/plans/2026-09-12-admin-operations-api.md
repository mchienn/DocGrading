...
- One user patch may change roles, account status, or both. The service locks the user row, canonicalizes roles, increments `revision` only when data changes, and records separate atomic `AuditEvent` rows for `ROLE_CHANGE`, `LOCK`, and `UNLOCK`, each with exact before/after values and the supplied reason.
...
- Role and lock-state changes persist one exact before/after audit per changed field in the same transaction; unchanged patches create no audit.
...
- `RUN_DATABASE_TESTS=1 uv run pytest -q tests/test_t018_admin_operations.py`: 6 passed.
- `RUN_DATABASE_TESTS=1 uv run pytest -q tests/test_t018_admin_operations.py tests/test_t009_job_recovery.py`: 27 passed.
- Full database-enabled suite on an isolated PostgreSQL 17 database: 370 passed in 491.08 seconds.
...
- No open finding. Coverage proves Admin cross-Course job visibility, unchanged Teacher Course scoping, user/job/audit filters and pagination, safe job detail, account role/lock transitions, same-row retry parity, and live database dashboard aggregates.

### Data Integrity & Integration

- No open finding. User mutation refreshes and locks current row state, increments one revision per effective patch, writes exact field-specific audits in the same transaction, and emits none for replay. Admin retry directly reuses the T-009 service, existing job row, document transition, audit, dispatch outbox, and post-commit dispatcher.
- T-018 needs no schema change. First full-suite attempt used the populated development database and correctly hit existing lossy-downgrade guards. Final rerun on an isolated PostgreSQL 17 database passed all 370 tests; validation database was removed afterward.
