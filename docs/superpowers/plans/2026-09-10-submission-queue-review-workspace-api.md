...
- Soft lock is one durable row per `Submission`. Acquisition locks `public.courses` with `FOR SHARE`, then `public.submissions` with `FOR UPDATE`; same reviewer refreshes it. Another reviewer receives holder name and expiry without acquiring it. TTL is 10 minutes. Heartbeat and every draft write refresh TTL. Every write locks and re-checks current owner after expiry evaluation. Admin may release any lock. `ADMIN_RELEASE` records release and holder details atomically only when an Admin releases another reviewer's active lock or an expired lock; an Admin releasing their own active lock follows normal owner behavior and creates no force-release audit.
