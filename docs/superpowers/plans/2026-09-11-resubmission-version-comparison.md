...
- `GET /api/v1/submissions/{submission_id}/versions` returns versions oldest first in `{items, page, page_size, total}`. `processing_status` is normalized to `QUEUED`, `PROCESSING`, `AWAITING_REVIEW`, or `ERROR`; publication metadata comes from the latest immutable `PublishedResultVersion` when visible to the caller.
