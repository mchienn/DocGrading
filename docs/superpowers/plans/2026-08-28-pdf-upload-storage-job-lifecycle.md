...
- `POST /api/v1/document-versions/{version_id}/complete` locks the `DocumentVersion` and Assignment together. The first completion is accepted only while the Assignment is `OPEN` and its optional deadline has not passed; an already completed document remains idempotently readable afterward. It performs `HeadObject`, rejects server-observed content type/size mismatches, and conditionally copies the staging object with `CopySourceIfMatch` into a random sealed key that was never exposed by the upload credential. Only that sealed key is stored for worker/download use. Completion then marks the document queued and creates/reuses exactly one job plus one dispatch outbox row in the same transaction. After commit the API makes a bounded Celery publish attempt; the lifespan poller retains and retries failed dispatches without changing the accepted response. A transient storage failure commits sanitized `PROCESSING_FAILED/STORAGE_UNAVAILABLE`; the same no-job document can retry completion without re-upload or a renewed presigned window while the Assignment gate still passes.
...
| Client-hint and server-side verification | Request schema bounds, `HeadObject`, ETag-conditional staging-to-sealed `CopyObject`, and bounded `GetObject` validation |
| PDF size / magic / scan-only errors | Stable error codes `PDF_TOO_LARGE`, `PDF_DECODED_TOO_LARGE`, `NOT_A_PDF`, `PDF_SCAN_ONLY`, `PDF_SCAN_ANALYSIS_UNSUPPORTED`; BR-07 recursively measures visible raster placements against crop, Form, and supported content clipping, and fails unsupported applied clipping closed as `PDF_SCAN_ANALYSIS_UNSUPPORTED`; raw/decoded-size and scan-geometry regression tests |
...
- [x] Add failing tests proving client-declared MIME/size are only hints, completion trusts server-observed object metadata/bytes, conditional copy pins the observed ETag, and the queued document points to a key absent from the presigned upload credential.
- [x] Implement initiation under a transaction/row lock; create no duplicate `Submission` or `DocumentVersion` under retry.
- [x] Implement completion as idempotent: lock the document and Assignment together; accept first completion only while the Assignment is `OPEN` and not past its optional deadline; already queued/processed returns the existing job even after later closure/deadline; conditional-copy the observed staging ETag to a random sealed key before first queueing; first completion creates/reuses the single job and its unique dispatch row in the same transaction, then makes a bounded post-commit publish attempt. Persist transient storage failure before 503 and allow only that `STORAGE_UNAVAILABLE`/no-job state to retry completion without re-upload or presign-expiry rejection.
